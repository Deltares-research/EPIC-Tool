...
class Command(BaseCommand):
    ...
    def _remove_migrations(self):
        ...
        for m_file in migrations_dir.glob("*.py"):
            if m_file.name != "__init__.py":
                self.stdout.write(self.style.NOTICE(f"Removing migration file: {m_file.name}")) 
                m_file.unlink()
    ...
    def _cleanup_db(self):
        ...
        self.stdout.write(self.style.SUCCESS("Successfully cleaned up previous database structure.")) 
